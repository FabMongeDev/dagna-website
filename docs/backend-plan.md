...
6. **Downloaded files can carry restrictive permissions (`-rw-------`) that survive the entire deploy pipeline without any error.** If an asset was downloaded (e.g. from a chat tool) and copied into `frontend/public/`, it may lack read permission for anyone but the owner. `rsync -a` preserves permissions exactly, so the file uploads "successfully" (no error, correct byte size) but the web server user can't read it, producing a silent `403 Forbidden` in production only. Always check `ls -la` before committing new binary assets; fix locally with `chmod 644` before rsyncing, or directly on the server via SSH if already deployed wrong.
7. **Hostinger's CDN caches aggressively (`cache-control: public, max-age=604800`, 7 days) and has multiple edge nodes** — after updating a static asset (image, favicon, etc.), different visitors may see old vs. new versions inconsistently for a while, since each request can hit a different edge node with its own cache state. If `curl` checks show inconsistent file sizes across repeated requests to the same URL, this is the likely cause, not a broken deploy. Fix: purge the CDN cache from hPanel (Websites → domain → CDN) after any static asset update, especially images. Hostinger's CDN also does its own automatic image optimization/WebP conversion, so served file sizes may legitimately differ slightly from the original uploaded file — that alone isn't a sign of a stale cache.
